...
import { ChatSession, Message, UserStats } from '../types';
...
export default function Home() {
  ...
  const simulateTyping = async (sessionId: string, fullText: string) => {
    ...
    const CHUNK_SIZE = isCode ? 180 : 50; 
    const DELAY = 1; 
    let currentText = "";
  ...
  const handleSendMessage = async () => {
    ...
    setSessions(prev => prev.map(s => 
      s.id === activeSession!.id ? { ...s, messages: [...s.messages, userMessage], lastModified: Date.now() } : s
    ));
    ...
    try {
      ...
      setSessions(prev => prev.map(s => 
        s.id === activeSession!.id ? { ...s, messages: [...s.messages, placeholder], lastModified: Date.now() } : s
      ));
  ...
  return (
    <div className="flex h-screen bg-slate-50 text-slate-900 overflow-hidden font-inter selection:bg-indigo-100">
      <aside className={`${isSidebarOpen ? 'translate-x-0 w-[280px]' : '-translate-x-full w-0'} fixed inset-y-0 left-0 z-50 bg-white border-r border-slate-200 transition-all duration-300 md:relative md:translate-x-0 overflow-hidden flex flex-col shadow-xl md:shadow-none`}>
        ...
        <div className="px-4 mb-4 space-y-3">
          <button onClick={() => createNewChat()} className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-bold transition-all shadow-md active:scale-95">
            <Plus size={18} strokeWidth={3} />
            <span className="text-sm">Obrolan Baru</span>
          </button>
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={14} />
            <input type="text" placeholder="Cari histori..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-9 pr-4 py-2 bg-slate-100 border-none rounded-xl text-xs font-semibold focus:ring-2 focus:ring-indigo-500/20 transition-all" />
          </div>
        </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <button key={tool.id} onClick={() => createNewChat(tool.id)} className="flex flex-col items-center justify-center gap-1.5 p-3 bg-white border border-slate-100 rounded-xl hover:border-indigo-200 hover:shadow-sm transition-all group">
                  <div className={`p-2 rounded-lg ${tool.color} group-hover:scale-110 transition-transform`}><tool.icon size={14} /></div>
          ...
          <div>
            ...
            <div className="space-y-1">
              {sortedSessions.map(session => (
                <div key={session.id} onClick={() => { setCurrentSessionId(session.id); if (window.innerWidth < 768) setIsSidebarOpen(false); }} className={`group flex items-center gap-2.5 px-3 py-2.5 text-xs rounded-xl cursor-pointer transition-all ${currentSessionId === session.id ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'}`}>
                  ...
                  <div className={`flex items-center gap-1 ${currentSessionId === session.id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                    <button onClick={(e) => { e.stopPropagation(); setSessions(prev => prev.map(s => s.id === session.id ? { ...s, isPinned: !s.isPinned } : s)); }} className="p-1"><Pin size={10} fill={session.isPinned ? 'currentColor' : 'none'} /></button>
                    <button onClick={(e) => { e.stopPropagation(); setSessions(prev => prev.filter(s => s.id !== session.id)); if(currentSessionId === session.id) setCurrentSessionId(null); }} className="p-1"><Trash2 size={10} /></button>
                  </div>
                </div>
              ))}
      ...
      <main className="flex-1 flex flex-col relative bg-white min-w-0 overflow-hidden">
        <header className="h-16 flex items-center justify-between px-6 border-b border-slate-100 sticky top-0 z-40 bg-white/80 backdrop-blur-md">
          <div className="flex items-center gap-4 min-w-0">
            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-2 bg-slate-100 rounded-lg text-slate-500 md:hidden"><Menu size={18} /></button>
            <h2 className="font-bold text-slate-900 truncate text-sm">{currentSession ? currentSession.title : 'Selamat Datang'}</h2>
          </div>
          <div className="flex items-center gap-3">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar px-4 md:px-10 py-8 space-y-10">
          {!currentSession || currentSession.messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center max-w-2xl mx-auto text-center space-y-8 animate-fade-in">
              <div className="w-20 h-20 rounded-3xl bg-indigo-600 flex items-center justify-center text-white shadow-xl ring-8 ring-indigo-50 animate-bounce"><Sparkles size={40} /></div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full pt-4">
                {[
                  ...
                  { title: "Debug Python", icon: BugIcon },
                  { title: "Bantu Desain Web", icon: Layout }
                ].map((item, i) => (
                  <button key={i} onClick={() => setInput(`Bantuin ${item.title.toLowerCase()} dong...`)} className="p-4 bg-white border border-slate-200 rounded-2xl text-left hover:border-indigo-500 hover:shadow-lg transition-all flex items-center gap-3 group">
            ...
            <div className="max-w-4xl mx-auto w-full space-y-10 pb-10">
              {currentSession.messages.map((msg, i) => (
                <div key={i} className={`flex gap-3 md:gap-5 animate-fade-in ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                  <div className={`w-9 h-9 md:w-10 md:h-10 rounded-xl flex-shrink-0 flex items-center justify-center text-xs font-bold shadow-md ${msg.role === 'user' ? 'bg-slate-900 text-white' : 'bg-indigo-600 text-white'}`}>
                  ...
                  <div className={`flex-1 min-w-0 ${msg.role === 'user' ? 'text-right' : ''}`}>
                    ...
                    <div className={`p-4 md:p-5 rounded-2xl shadow-sm overflow-hidden break-words ${msg.role === 'user' ? 'bg-indigo-600 text-white rounded-tr-none inline-block max-w-full text-left' : 'bg-white text-slate-800 rounded-tl-none border border-slate-100 block w-full'}`}>
              ...
              {isProcessing && (
                <div className="flex gap-4 animate-pulse">
                  <div className="w-10 h-10 rounded-xl bg-indigo-50 flex items-center justify-center text-indigo-300 border border-indigo-100"><Sparkles size={16} /></div>
        ...
        <div className="p-4 md:p-8 bg-white/50 backdrop-blur-md border-t border-slate-50">
          <div className="max-w-4xl mx-auto relative bg-white border border-slate-200 rounded-2xl md:rounded-[2rem] shadow-xl p-1.5 focus-within:ring-4 focus-within:ring-indigo-50 transition-all">
            ...
            <div className="flex items-end gap-1 px-2 py-1.5">
              <button onClick={() => fileInputRef.current?.click()} className="p-2.5 text-slate-400 hover:text-indigo-600 transition-colors flex-shrink-0"><ImageIcon size={20} /></button>
              <input type="file" ref={fileInputRef} onChange={handleImageUpload} className="hidden" accept="image/*" />
              <textarea ref={textAreaRef} rows={1} value={input} onChange={(e) => { setInput(e.target.value); e.target.style.height = 'auto'; e.target.style.height = Math.min(e.target.scrollHeight, 200) + 'px'; }} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSendMessage(); } }} placeholder="Tanya apa aja ke Soraa..." className="flex-1 py-2.5 px-2 bg-transparent focus:outline-none resize-none text-[14px] font-medium text-slate-800 min-h-[44px] max-h-[300px] custom-scrollbar" />
              <button onClick={handleSendMessage} disabled={isProcessing || (!input.trim() && !selectedImage)} className={`p-2.5 rounded-xl transition-all flex-shrink-0 ${isProcessing || (!input.trim() && !selectedImage) ? 'bg-slate-100 text-slate-300' : 'bg-slate-900 text-white shadow-lg hover:-translate-y-0.5 active:scale-95'}`}>
                <Send size={20} />
              </button>
            </div>
          </div>
          <p className="mt-4 text-center text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] hidden sm:block">Soraa Pro AI • {userStats.isUnlimited ? 'Unlimited Developer Access' : `Limit ${DAILY_LIMIT} Pesan Per Hari`}</p>
        </div>
      </main>
    </div>
  );
